...
dividediv.className = `divide`
document.body.appendChild(dividediv)
...
for (const simpson of simpsons){
    ...
    simmember.className = `member`
    document.body.appendChild(simmember);
    for(prop in simpson){
        ...
        simmember.appendChild(propdiv)
        if (prop === `photo`){
            let img = document.createElement(`img`)
            img.src = `${simpson[prop]}`

            propdiv.appendChild(img)

        }else {
            propdiv.innerText = `${simpson[prop]}`
        }
    }

}
let dividedivclone = dividediv.cloneNode(false)
document.body.appendChild(dividedivclone)
...
for (i of courses) {
    ...
    divElement.setAttribute(`class`, `basediv`)

    document.body.appendChild(divElement)
    for (const el in i) {
        let divElementLevTwo = document.createElement(`div`)


        divElement.appendChild(divElementLevTwo)
        divElementLevTwo.className = `${[el]}`
        if (divElementLevTwo.className === `modules`) {
            for (let li in i.modules){
                console.log(i.modules[li])
                let lili = document.createElement(`li`)
                lili.innerText = i.modules[li]


                divElementLevTwo.appendChild(lili)
            }
            // console.log(i.modules)
        } else {
            // console.log(i[el])
            // divElementLevTwo.innerText = `${[el]}`
            divElementLevTwo.innerText = `${i[el]}`
        }

        // divElementLevTwo.innerText = `${courses[`${el}`]}`
    }
}
